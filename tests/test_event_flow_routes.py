...
def test_skip_bow_reserves_an_upper_corridor():
    ...
    assert route.path == "M110,50 C134,50 134,-4 160,-4 S186,-4 186,130 S210,130 210,130"
...
def test_back_sag_reserves_a_lower_corridor():
    ...
    assert route.path == "M210,130 C186,130 186,184 160,184 S134,184 134,50 S110,50 110,50"
...
def test_c_loop_uses_the_requested_exterior_side():
    ...
    assert upward.label_anchor[0] == 186.0
    assert upward.bounds[0] == 186.0
    downward = route_c_loop((210, 0, 100, 60), DST, offset=24, side="right")
    assert downward.label_anchor[0] == 334.0
    assert downward.bounds[2] == 334.0
